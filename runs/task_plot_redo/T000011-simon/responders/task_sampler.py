...
class TaskSamplerResponder:
    """Task-local sampler for Simon task simulations."""
    ...
    def __init__(
        ...
        *,
        left_key: str = "f",
        right_key: str = "j",
        p_correct_congruent: float = 0.91,
        p_correct_incongruent: float = 0.80,
        p_miss: float = 0.04,
        rt_congruent_mean_s: float = 0.31,
        rt_incongruent_mean_s: float = 0.38,
        rt_sd_s: float = 0.06,
        rt_min_s: float = 0.12,
    ...
    def act(self, obs: Observation) -> Action:
        ...
        if phase in {"instruction_text", "block", "goodbye", "block_feedback"}:
            ...
            if key is None:
                return Action(key=None, rt_s=None, meta={"source": "simon_sampler", "reason": "no_valid_key"})
            return Action(key=key, rt_s=self.continue_rt_s, meta={"source": "simon_sampler", "phase": phase})

        if phase not in {"stimulus", "simon_response"} or self._rng is None:
            return Action(key=None, rt_s=None, meta={"source": "simon_sampler", "phase": phase, "reason": "withhold"})

        condition = str(obs.condition_id or "").strip().lower()
        congruent = condition in ("red_left", "blue_right")
        correct_key = self.left_key if condition.startswith("red_") else self.right_key

        if float(self._rng.random()) < self.p_miss:
            return Action(key=None, rt_s=None, meta={"source": "simon_sampler", "condition": condition, "outcome": "miss"})
        ...
        rt_s = self._sample_rt(self.rt_congruent_mean_s if congruent else self.rt_incongruent_mean_s)
        deadline = self._deadline(obs)
        if deadline is not None and rt_s > deadline:
            return Action(key=None, rt_s=None, meta={"source": "simon_sampler", "condition": condition, "outcome": "late"})

        return Action(key=key, rt_s=rt_s, meta={"source": "simon_sampler", "condition": condition, "outcome": outcome})
