...
def run(options: TaskRunOptions):
    """Run REST task in human/qa/sim mode with one auditable flow."""
    task_root = Path(__file__).resolve().parent
    cfg = load_config(str(options.config_path))
    print(f"[REST] mode={options.mode} config={options.config_path}")
    ...
    with runtime_scope:
        ...
        instruction = StimUnit("instruction", win, kb, runtime=trigger_runtime).add_stim(stim_bank.get("general_instruction"))
        if bool(getattr(settings, "voice_enabled", True)) and options.mode not in ("qa", "sim"):
            instruction.add_stim(stim_bank.get("general_instruction_voice"))
        ...
        goodbye = StimUnit("block", win, kb, runtime=trigger_runtime).add_stim(stim_bank.get("good_bye"))
        if bool(getattr(settings, "voice_enabled", True)) and options.mode not in ("qa", "sim"):
...
def main() -> None:
    ...
    options = parse_task_run_options(
        task_root=task_root,
        description="Run REST task in human/qa/sim mode.",
